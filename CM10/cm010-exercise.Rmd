...
```{r}
time %>% 
  semi_join(album, by = c("song", "artist_name")) # drop other columns: "filter"
                # use both "song" and "artist_name", which appear in both time and album datasets

time %>% 
  inner_join(album, by = c("song", "artist_name")) # keep other columns: "mutate"
```
...
```{r}
time %>% 
  semi_join(album, by = "song") # I think here should be anti_join
```
...
```{r}
intersect(y, z) # both two lines can work
inner_join(y, z)
```
...
```{r}
setdiff(y, z) # both two lines can work
anti_join(y, z)
```
