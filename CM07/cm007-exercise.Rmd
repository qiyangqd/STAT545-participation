...
gapminder %>% 
  ...
  summarize(min_life = min(lifeExp),
            country = country[lifeExp == min_life]) %>% # keep these columns only
  arrange(min_life)
```
...
gapminder %>% 
  group_by(country) %>% 
  arrange(year) %>% 
  mutate(rel_growth = pop - first(pop)) %>% # first value of pop
...
Testing another dataset in class (can hardly remember why I added this line in class)
```{r}
