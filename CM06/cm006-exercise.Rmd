...
```{r}
select(gapminder, year, lifeExp, country) # select: columns
```
...
# Better way:
select(gapminder, country:lifeExp) # no need to type all words
```
...
```
we can rearrange by typing in the desired order.
...
select(gapminder, country, gdpPercap:continent)
...
```{r}
arrange(gapminder, year) # change the order of rows by a variable in a column
```
...
# we are not storing the change of order in gapminder: gapminder doesn't change actually, only output changes
gapminder %>% 
  select(year, lifeExp, country) %>%
  arrange(year, lifeExp)

# arrange(select(gapminder, year, lifeExp, country), year, lifeExp) is inconvenient to read
```
...
```{r}
gapminder %>%
  filter(pop > 100000000) # filter out rows
```
...
```{r}
gapminder %>% 
  mutate(cc = paste(country, continent, sep = ", ")) # Asked TA about this
