...
- what class is  `continent`(a factor or character)? factor (class())
- How many levels? What are they? 5(nlevels()), "Africa"   "Americas" "Asia"     "Europe"   "Oceania" (levels())
- What integer is used to represent factor "Asia"? 3 (str())
...
- Filter `gapminder` dataset by 5 countries of your choice. How many levels are in your filtered dataset? 142
...
# h_gap_dropped <- h_gap %>% 
#   forcats::fct_drop()  this won't work as it works on a factor only
...
##  order from high to low according to the other variable
gapminder %>%
  ggplot() +
...
## both quantitative x and y are quantitative
...
gapminder %>%  
  ggplot() +
  geom_bar(aes(fct_relevel(continent, "Oceania"))) +# Oceainia at the first position
  coord_flip()+
  theme_bw()
```
