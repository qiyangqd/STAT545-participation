...
ggplot(mpg, aes(cty, hwy)) + 
  geom_jitter(size = 1, alpha = 0.5) + # make dots not on a straight line
  geom_smooth(method = "lm") + # make grey area smaller
  theme_bw()
```
...
```{r, fig.width = 5, fig.height = 2}
gapminder %>% 
  # group_by(country) %>% won't be recognized by ggplot
  ggplot(aes(year, lifeExp, group = country, colour = country == "Rwanda")) +
  geom_line(alpha = 0.3) +
  scale_color_discrete("", labels = c("Other", "Rwanda")) #change the title and text of the legend 
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  scale_x_log10(labels = scales::comma_format()) + # change the label of the axis
  facet_wrap(~ continent, scales = "free_y") # split the plot by continents, "free" gives each plot x- and y-axis
```
...
gapminder %>% 
  ...
  ggplot(aes(gdpPercap, lifeExp, size = pop, fill = continent)) + # added "size = pop". color = gives color. fill = fill in circles with colors
  facet_wrap(~ continent, nrow = 2) + # added "nrow = 1"
  geom_point(alpha = 0.5, shape = 21) +
  scale_x_log10(labels = scales::comma_format()) +
  scale_size_area() # added
```
...
tribble(
  ...
  ggplot(aes(x, y)) + 
  geom_path(arrow = arrow())+# Figured out in previous session
...
gapminder %>% 
  ...
  geom_jitter() +
  geom_boxplot() +
  coord_flip() # change y- and x- axis
```
...
mtcars %>% 
  ...
  ggplot(aes(cyl,..prop..)) +
  geom_bar(aes(fill = transmission), position = "dodge") +  # color changes the color of border
...
ggplot(hair_eye, aes(Hair, Eye)) +
  geom_tile(aes(fill = n)) # fill changes inside while color only changes border
```
