...
- Main takeaway: We don't know the sample size or SD; the difference has been "exaggerated" (the gaps between them are less than 1 hour).      Another tip for more effective visualization: numbers on axis don't need to appear again beside bars.
...
- Main takeaway: 1. Scientific methods are needed to interpret the plot (if people don't believe in global warming they will use the plot above!). 2. When we are making the plot, we can decide whether the axis should start from zero or not according to our needs.
...
- Main takeaway: The y axis starts from the maximum value instead of zero...So the gun shot actually rose. Other tips to make effective visualization: 1. Colors matter. Choose the color of background and plot wisely. Don't be creative about the axis or colors: make sure it fits normall people's habits.
...
# successfully generated a public link: https://plot.ly/~qiyangqd/1/
```
...
2. codes for different graphs

## case study
1. case study 1: scales of y axis on the right and left are quite different. 100-fold. If made on the same scale autism should just stay static.
2. case study 2: how come people finish 100m in negative time
