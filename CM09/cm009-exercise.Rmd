...
```{r}
(lotr_wide <- lotr %>% 
  pivot_wider(id_col = c(-Race, -Words),  # columns that you want to keep
              names_from = Race,  # names of new columns that your want to create
              values_from = Words)) # values of new columns that you want to create
```
...
```{r}
lotr_wide %>% 
  pivot_longer(cols = c(-Film, -Gender), # columns you want to condense in to one
               names_to  = "Race", # what's the name of the new column
               values_to = "Words") # where will values of old columns go 
```
...
# not the same as what is covered in class
